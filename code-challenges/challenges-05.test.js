...
const templateWithJQuery = () => {
  starWarsPeople.forEach( person => {
    ...
    $templateCopy.find('h2').text(person['name']);
    $templateCopy.find('h3').text(person['height']);
    $templateCopy.find('p').text(person['eye_color']);
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < length; i++){
    result.push(str);
    // https://stackoverflow.com/questions/4564414/delete-first-character-of-a-string-in-javascript splicing from 1 to str.length-1 was producing strange results
    str = str.substring(1);
  }
  result.push('');
  return result;
...
const extractVowels = (str) => {
  ...
  for (let i = 0; i < char.length; i++) {
    if (char[i] == 'a' || char[i] == 'e' || char[i] == 'i' || char[i] == 'o' || char[i] == 'u'){
      vowelStr+=char.splice(i,1);
      i--;
    }
  }
  arr.push(char.join(''));
  let sorter = vowelStr.split('');
  sorter.sort();
  arr.push(sorter.join(''));
  return arr;
};
