...
const firstLetters = (arr) => {
  return arr.map(str => str.match(/[A-Z]/i)[0]);
};
...
const findHappiness = (arr) => {
  return arr.reduce( (newArr, str) => /\:\)/.test(str) ? newArr.concat(str) : newArr, []);
};
...
const onlyOddChars = (str) => {
  // let newStr = '';
  // let arr = str.split('');
  // arr.forEach( (char, index) => index%2 != 0 ? newStr+=char : newStr );
  // return newStr;
  return str.split('').map( (char, index) => index%2 ? char : '' ).join('');
};
...
const allHappy = (arr) => {
  return arr.reduce((bool, str) => /\:\)/.test(str) ? bool : false, true);
};
...
const findAnything = (arr, target) => {
  return arr.reduce((targets, str) => str.includes(target) ? targets.concat(str) : targets, []);
};
...
const findEvery = (arr, target) => {
  return arr.reduce((bool, str) => str.includes(target) ? bool : false, true);
};
...
const unenrollBrook = (arr) => {
  return arr.map(inner => inner.reduce( (people, name) => /(brook)/i.test(name) ? people : people.concat(name), []))
};
...
const sortByDay = (arr) => {
  return daysOfWeek.map(day => arr.reduce( (list, events) => events.includes(day) ? list.concat(events) : list, []));
  // return arr.reduce( (sorted, event) => {
  //   if (/(monday)/i.test(event)) 
  //     sorted[0].push(event);
  //   if (/(tuesday)/i.test(event)) 
  //     sorted[1].push(event);
  //   if (/(wednesday)/i.test(event)) 
  //     sorted[2].push(event);
  //   if (/(thursday)/i.test(event)) 
  //     sorted[3].push(event);
  //   if (/(friday)/i.test(event)) 
  //     sorted[4].push(event);
  //   if (/(saturday)/i.test(event))
  //     sorted[5].push(event);
  //   if (/(sunday)/i.test(event)) 
  //     sorted[6].push(event);
  //   return sorted;
  // }, [[], [], [], [], [], [], []]);
};
...
const characterByIndex = (arr) => {
  return arr.reduce( (ret, str, idx) => ret.concat(str[idx]), []);
};
