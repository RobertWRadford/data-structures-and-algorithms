...
function getCurrentEvents(request, response){
  ...
  response.status(200).send(curEvents);
}
...
const returnNames = (arr) => {
  return arr.reduce(function(names, obj){names.push(obj.name); return names;}, []);
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce(((total, obj) => obj.hasOwnProperty('children') ? total+=obj.children.length : total), 0);
};
...
const calculateAverage = (arr) => {
  let tallies = arr.reduce(function(average, num) {average.count += 1; average.sum += num; return average}, { count: 0, sum: 0 });
  return tallies.sum / tallies.count;
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce( (count, num) => isPrime(num) ? count+=1 : count, 0)
};
...
const extractStat = (statName, arr) => {
  let checkStats = arr.reduce((ret, obj) => { if(obj.stat.name === statName){ret.push(obj)} return ret;}, []);
  return checkStats.length ? checkStats[0] : null;
};
...
const extractChildren = (arr) => {
  let aNames = arr.filter(character => /[aA]/.test(character.name));
  return aNames.reduce((children, person) => { if(person.hasOwnProperty('children')){person['children'].forEach(child => children.push(child))} return children;}, [])
};
