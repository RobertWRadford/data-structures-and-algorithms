...
let biggerThanLuke = (arr) => {
  let heavyArr = arr.filter(item => item.mass > 77);
  return heavyArr.reduce((ret, cur) => ret.concat(cur.name), []).join(' - ');
};
...
const sortBy = (property, arr) => {
  if (typeof(eval(`arr[0].${property}`)) == 'number') {
    return arr.sort((a, b) => eval(`a.${property} - b.${property}`));
  } else {
...
const detectTicTacToeWin = (board) => {
  function isSame(p1, p2, p3){
    return board[p1[0]][p1[1]] == board[p2[0]][p2[1]] && board[p1[0]][p1[1]] != '' ? board[p1[0]][p1[1]] == board[p3[0]][p3[1]] ? true : false : false;
  }
  const tripPairs = [
                    [[0, 0], [0, 1], [0, 2]],
                    [[1, 0], [1, 1], [1, 2]],
  ...
  return tripPairs.reduce( (bool, triple) => isSame(triple[0], triple[1], triple[2]) ? true : bool, false);
};
