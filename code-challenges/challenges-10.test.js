...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => res.send('Greetings, fellow humans.'));
  app.get('/aboutme', (req, res) => res.send('I\'m Robert, I was in the military for 5 years and I like to code in Python.'));
  app.get('/favoritefoods', (req, res) => res.send(['Chicken Pot Pie', 'Pizza', 'Steak']));
...
const count = (target, input) => {
  return input.reduce(((tally, item) => tally+=item.reduce(((tal, ite) => ite == target ? tal+=1 : tal), 0)), 0)
};
...
const totalSum = (input) => {
  return input.reduce(((tally, item) => tally+=item.reduce(((tal, num) => tal+=num), 0)), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = input.map(arr => arr.filter(item => typeof(item) == 'number' && item % 5 == 0));
  return newArr.map(outer => outer.map( inner => Math.pow(2, inner)));
};
...
let findMaleAndFemale = (data) => {
  let binaryChars = data.filter(obj => obj.gender == 'male' || obj.gender == 'female');
  let binaryCharsNames = binaryChars.map(obj => obj.name);
  return binaryCharsNames.join(' and ');
};
...
let findShortest = (data) => {
  ...
  let shortestChar = data.reduce(((char, item) =>
    parseInt(item.height) <= char.height ? {name: item.name, height: parseInt(item.height)} : char), //update init value if shorter
    {name: data[0].name, height: data[0].height} ); //init value
  return shortestChar.name;
  // let shortest = parseInt(data[0].height);
  // var onlyShortest;
  // do {
  //   onlyShortest = data.filter(obj => {if (parseInt(obj.height) <= shortest) {shortest = parseInt(obj.height); return true;} else {return false;}});
  // } while (onlyShortest.length > 1)
  // return onlyShortest[0].name;
};
